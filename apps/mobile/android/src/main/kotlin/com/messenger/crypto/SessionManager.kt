// SessionManager.kt — E2E Session Manager (Android).
// Зеркало SessionManager.swift (iOS). Совместим с web-клиентом (session-web.ts).
//
...
// ── Ratchet state ─────────────────────────────────────────────────────────────
// ByteArray fields хранятся как base64-строки для простой JSON-сериализации.

@Serializable
private data class RatchetState(
    val dhSendPublic: String,           // base64
    val dhSendPrivate: String,          // base64
    val dhRemotePublic: String? = null, // base64 | null
    val rootKey: String,                // base64
    val sendChainKey: String? = null,   // base64 | null
    val recvChainKey: String? = null,   // base64 | null
    val sendCount: Int,
    val recvCount: Int,
...
// ── KeyAccess interface ───────────────────────────────────────────────────────
// Позволяет подставить тестовую реализацию без зависимости на Android Keystore.
...
class SessionManager(
    ...
    // ── Direct message encrypt (one call per device) ──────────────────────────
    ...
    fun encryptForDevice(peerId: String, deviceId: String, bundle: DeviceBundle, plaintext: String): String {
        ...
        var wireExtra: Triple<String, Int?, String>? = null   // ek, opkId, ikPub
    ...
    : Pair<RatchetState, Triple<String, Int?, String>> {
        ...
        // initRatchet initiator: DH with Bob's SPK to derive initial root + send chain
    ...
    private fun ratchetDecrypt(state: RatchetState, message: EncryptedMsg): Pair<String, RatchetState> {
        ...
        // DH ratchet if remote DH public changed

...
## #3 Финализация `docs/main/` — P1  ✅ done 2026-06-23 (PR-DOCS-1)

**Контекст:** Файлы переработаны по плану `docs-main-update-plan.md`, но чеклист не отмечен выполненным. Нужна сверка. **Сверка завершена 2026-06-23 — расхождений не выявлено.**
...
- **3.4 Сверка `deployment.md`** — ENV-переменные против `server/cmd/server/config.go`, Docker Compose, VAPID, STUN/TURN, BEHIND_PROXY. ✅ 2026-06-23 — все 33 ENV-переменные из `config.go` присутствуют в `technical-documentation.md` (§10.1–10.7), deployment.md и usersguid.md с верными дефолтами; расхождений нет.
- **3.5 Внутренние ссылки** — проверить кросс-ссылки между файлами `docs/main/*` (grep `](./` и `](../`). ✅ 2026-06-23 — относительных кросс-ссылок между файлами `docs/main/*` нет, битых ссылок нет.
...
- [x] Все четыре файла `docs/main/*.md` актуальны (сверка ENV против `config.go` и внутренних ссылок завершена 2026-06-23, PR-DOCS-1 done).

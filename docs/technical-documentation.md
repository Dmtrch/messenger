...
### 28.1 Device model (архитектурный долг)

Таблица `devices` добавлена, но:

- `identity_keys` всё ещё по PK `user_id` — `UpsertIdentityKey` перезаписывает ключи;
- `PopPreKey` не фильтрует по `device_id` — OPK смешаны между устройствами;
- `POST /api/keys/register` не идемпотентен: нет device fingerprint, каждый вход создаёт новое устройство.

Полноценный multi-device требует миграции PK `identity_keys` на `(user_id, device_id)`.
...
### 28.3 Offline history

Zustand store не персистируется в IndexedDB. При перезагрузке страницы история подгружается заново с сервера. Оффлайн-просмотр истории недоступен.
...
## 28.7 Аудио/видео звонки отсутствуют

В текущей реализации нет ни одного компонента для звонков:

- нет WS-событий сигнализации (`call_offer`, `call_answer`, `ice_candidate` и т.д.)
- нет хранения состояния активных звонков в Hub
- нет `RTCPeerConnection` на клиенте
- нет UI звонков
- нет STUN/TURN конфигурации
- нет endpoint `GET /api/calls/ice-servers`

Минимальный объём для звонков 1-на-1:

- **Backend** — 4–5 новых case в WS readPump для транзитной пересылки сигнальных событий; in-memory map активных звонков в Hub
- **Frontend** — `RTCPeerConnection`, UI входящего/активного звонка, обработка ICE
- **Конфигурация** — STUN URL в env (или вшить публичный); опционально TURN

Групповые звонки требуют отдельного SFU-сервиса (LiveKit) и не вписываются в self-hosted монолит без существенного усложнения инфраструктуры.

## 29. Рекомендации по дальнейшему развитию

### 29.1 Приоритет 1 — Device model

Мигрировать `identity_keys` на PK `(user_id, device_id)`, добавить device fingerprint в `POST /api/keys/register`, фильтровать OPK по device.

### 29.2 Приоритет 2 — Offline history

Ввести IndexedDB persistence для `chatStore`: сохранять сообщения при получении/отправке, при запуске читать из кэша, затем делать background sync.
...
### 29.6 Аудио/видео звонки

- **Этап 9.1** — сигнализация: добавить 6 WS-событий, транзитная пересылка, in-memory call state в Hub
- **Этап 9.2** — клиент: `RTCPeerConnection`, UI звонка, обработка ICE negotiation
- **Этап 9.3** — TURN: coturn, временные credentials через `GET /api/calls/ice-servers`
- **Этап 9.4** — групповые звонки: LiveKit SFU как отдельный Docker-сервис

### 29.5 Эксплуатация

- versioned migrations вместо runtime ALTER TABLE;
- конфигурационный файл сервера;
- deployment guide для Cloudflare Tunnel.
...
Этапы 1–5 плана `v1-gap-remediation.md` закрыты. Реализованы:
...
Незакрытые зоны (этапы 6–8):

- полноценный multi-device (device-level key binding);
- Sender Key ротация при смене состава группы;
- offline history (IndexedDB persistence);

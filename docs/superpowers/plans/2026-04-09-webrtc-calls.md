...
- [ ] **Step 1: Добавить `callSession` тип и поля Hub**
...
- [ ] **Step 2: Инициализировать `calls` в `NewHub`**
...
- [ ] **Step 3: Расширить `inMsg` полями для звонков**
...
- [ ] **Step 4: Создать тестовый файл с хелперами**
...
- [ ] **Step 5: Запустить сборку для проверки компиляции**
...
- [ ] **Step 1: Написать failing-тест для handleCallOffer**
...
- [ ] **Step 3: Реализовать `handleCallOffer`**
...
- [ ] **Step 1: Написать failing-тест для ICE relay**
...
- [ ] **Step 3: Реализовать `handleIceCandidate`**
...
- [ ] **Step 4: Реализовать `cleanupCallsForUser` и изменить `unregister`**
...
- [ ] **Step 5: Добавить call-case в `readPump` switch**
...
- [ ] **Step 6: Запустить все тесты — убедиться что PASS**
...
- [ ] **Step 3: Создать `server/cmd/server/calls.go`**
...
- [ ] **Step 5: Зарегистрировать роут в `main.go`**
...
- [ ] **Step 6: Собрать и запустить все серверные тесты**
...
- [ ] **Step 1: Добавить call-типы в `types/index.ts`**
...
- [ ] **Step 2: Добавить `api.getIceServers()` в `client/src/api/client.ts`**
...
- [ ] **Step 1: Создать `client/src/store/callStore.ts`**
...
- [ ] **Step 1: Создать `client/src/utils/ringtone.ts`**
...
- [ ] **Step 1: Создать `client/src/hooks/useWebRTC.ts`**
...
- [ ] **Step 1: Создать `client/src/hooks/useCallHandler.ts`**
...
- [ ] **Step 1: Добавить call-фреймы в switch внутри `useMessengerWS.ts`**
...
- [ ] **Step 1: Создать `CallOverlay.module.css`**
...
- [ ] **Step 2: Создать `CallOverlay.tsx`**
...
- [ ] **Step 2: Обновить `ChatWindowPage` (или `ChatWindow`) для приёма `initiateCall` пропа**
...
- [ ] **Step 3: Добавить кнопки звонка в `ChatWindow` header**
...
- [ ] **Step 4: Проверить компиляцию и lint**
...
- [ ] Alice звонит Bob → Bob видит incoming call overlay с ringtone
- [ ] Bob принимает → оба видят active call, медиа идёт P2P
- [ ] Bob отклоняет → Alice видит toast "Звонок отклонён"
- [ ] Bob игнорирует → через 30 сек оба клиента сбрасываются в idle
- [ ] Alice звонит занятому Bob → Alice видит "Абонент занят"
- [ ] Alice отключается во время звонка → UI Bob сбрасывается
- [ ] TURN не настроен → `/api/calls/ice-servers` возвращает только STUN
- [ ] TURN настроен → `/api/calls/ice-servers` возвращает валидные временные credentials
- [ ] Переключение камеры/микрофона работает во время активного звонка
- [ ] Аудио-звонок не запрашивает видеодорожку
- [ ] `getUserMedia` denied → сообщение об ошибке, звонок не инициируется

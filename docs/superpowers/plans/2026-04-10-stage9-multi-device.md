...
**Дата:** 2026-04-10  
**Этап:** 10  
**Статус:** ✅ Реализовано — ветка feature/stage9-multi-device смержена в main (2026-04-12)  
...
- [x] `GET /api/keys/:userId` возвращает `{ devices: [...] }` — один entry на активное устройство
- [x] Два браузера одного пользователя получают сообщения независимо
- [x] ratchet-сессии хранятся по `peerId:deviceId` — не конкурируют
- [x] `senderDeviceId` передаётся в WS message payload
- [x] Клиент корректно расшифровывает сообщения от обоих устройств одного пира

### Non-Functional

- [x] Migration #9 идемпотентна (тест в `migrate_test.go`)
- [x] `PopPreKey` не смешивает OPK разных устройств
- [x] `type-check` и `lint` проходят без ошибок
- [x] Go тесты `./...` проходят

### Граничные случаи

- [x] Пользователь без зарегистрированных устройств → `GET /api/keys/:userId` → 404
- [x] OPK исчерпаны для одного устройства → bundle без `opkPub` (допустимо)
- [x] Пользователь с одним устройством — поведение не должно измениться
